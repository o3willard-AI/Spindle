...
# ── CHANGE 2: Change shared_buffers from 128MB to 512KB (brutal tuning) 
...
echo "[DONE] shared_buffers changed to 512kB"
echo ""
...
changes:
  ...
  - file:/etc/postgresql/16/main/postgresql.conf
    ...
    old_value:128MB
    new_value:512kB
    backup:/etc/postgresql/16/main/postgresql.conf.bak.${TIMESTAMP}
  - object:database
    old_name:${DB_NAME}
    new_name:${CHANGED_DB}
restore_commands:
  - cp /etc/postgresql/16/main/postgresql.conf.bak.${TIMESTAMP} /etc/postgresql/16/main/postgresql.conf
