...
fleet-01/02/03 (cinc-client, Ubuntu)
   │  data_collector['server_url'] = http://192.168.101.101:8081
   ▼
twin-write proxy (:8081)  ──► SPINDLE  http://192.168.101.101:8080   (works: 202 + archive)
   └──► CINC SERVER        https://192.168.101.220                    (server data_collector DISABLED → 405)
```
...
Dashboard `http://192.168.101.101:8081/health` at trace time:
...
`https://192.168.101.220` with the required `x-data-collector-token` header. The server
...
1. `CINC_SERVER_URL` was `http://192.168.101.220:443` — plain HTTP to an HTTPS-only server
   (→ 400). Fixed to `https://192.168.101.220` in the systemd unit.
