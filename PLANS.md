...
**Fix:** Bulk revocation uses single UPDATE on in-memory store (single lock + iterate). Expiry cleanup via `cleanup_expired_tokens()`. Sampled last_used_at prevents excessive DB writes.
...
**Requirements:** CMP-09
**Build:** Reports derived from restored archives carry `verification_status: "verified" | "unverified"` marker in attestation. Restore session metadata includes verification result. Every export from restored data: manifest includes `source_session_verification: "verified"` or `source_session_verification: "unverified"`.
**Verify:** Verified archive → export → attestation shows "verified". Unverified archive → export → attestation shows "unverified". Report header includes verification status.
**Fix:** Verification status cascades: unverified source → everything derived is unverified.
**Scale:** Restore session TTL includes verification status expiry.

### M4-15: C11 Parquet export
**Requirements:** ARC-01, ARC-02, ARC-03
**Build:** `spindle-archive::ParquetExporter` using `parquet` + `arrow` crates. Weekly partitions: one archive set = one directory with `runs.parquet`, `resource_events.parquet`, `control_results.parquet`, `nodes.parquet`, `schema.json` (Arrow schema + Spindle schema version). zstd compression level 3. Schema documented in `docs/archive-schema.md`. Export job: idempotent (check if week already exported), snapshot read at start time.
**Verify:** Export → load in DuckDB (`SELECT * FROM 'runs.parquet'`) → correct row count. DuckDB can answer: "how many failed runs in week X" → matches API query. Schema evolution: v1 schema documented.
**Fix:** Parquet row group size: 100k rows for balance of queryability and file size.
**Scale:** 20k-node weekly archive: ~245GB raw → ~40GB zstd compressed.

### M4-16: C11 Signed manifest + verification tool
**Requirements:** ARC-04, ARC-05, ARC-06, ARC-07, ARC-08, ARC-09
**Build:** Manifest: JSON with `manifest_version: 1`, `archive_week: "2026-W01"`, `exported_at: RFC3339`, `record_counts: {runs: N, resource_events: M, ...}`, `file_hashes: {SHA256: ...}`, `source_raw_digests: [...]`, `schema_version: 1`, `signing_key_id: "..."`. Manifest stored in `manifests` table (retained forever — ARC-05). Signed by C9. Export destination: S3-compatible or local path (configurable). CLI: `spindle archive export --week=2026-W01 --dest=s3://bucket/` and `spindle archive verify --archive=<path>`. Post-export: verify manifest → verify files against manifest → delete exported hot/warm rows. Failure at any point = no deletion (ARC-09).
