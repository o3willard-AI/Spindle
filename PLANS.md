...
**Requirements:** OPS-01
**Status:** ✅ Complete
**Build:** Three separate binaries (`spindle-server`, `spindle-worker`, `spindle-cli`) sharing a single `spindle.toml` config file. `spindle-server --validate-config` validates config and exits 0 (valid) or 1 (invalid with specific field errors like "database.url"). `spindle-worker --validate-config` for worker config validation. `check_port_available()` in `spindle_server::check_port_available` — binds test socket to detect port conflicts before startup (exit 3 on conflict). `Dockerfile` with multi-stage build (Rust 1.82 builder → debian:bookworm-slim runtime), single image for both server and worker, entrypoint defaults to `spindle-server` (override with `docker run spindle:latest worker`). Config file at `/config/spindle.toml`, env `SPINDLE_CONFIG` override. CLI config also reads shared `spindle.toml` with `[profiles]` table. **Verify:** 33 tests (46 CLI + 2 port + 4 config validation + 2 shared config + 3 binary smoke), `cargo build --workspace` green. **Scale:** Future path documented: `spindle server` / `spindle worker` subcommands on unified binary — not built yet.
...
**Verify:** `cargo test -p mcp-server` (7) + `-p spindle-mcp` (7) green. Live: `spindle-mcp serve --namespace spindle-query --api-url http://192.168.101.101:8080` → tools/list shows 11 query tools, list_nodes returns 50-node fleet page (total_count 567) with envelope.
