...
Polls Spindle's API at `198.51.100.101:8080` every 30 seconds and tracks state changes over time. When Mark's chaos cycle runs, it captures: node drifts out of compliance → InSpec detects → Cinc converges → compliance restored.
...
python run.py --api-base http://198.51.100.101:8080 --token spindle-dev-token --interval 15
