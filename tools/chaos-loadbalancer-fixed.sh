...
# ── CHANGE 2: Change health check interval from 10s to 60s (broken detection)
echo "[CHANGE 2] Changing health check interval to 60s..."
# Template renders `default-server inter 10s`; break it to 60s.
sed -i 's/default-server inter 10s/default-server inter 60s/' "$HAPROXY_CFG"
grep 'default-server inter' "$HAPROXY_CFG" | head -5 || true
systemctl reload haproxy 2>/dev/null || true
echo "[DONE] Health check interval changed: 10s → 60s"
echo ""

# ── CHANGE 3: Set dangerously low client timeout causing connection drops ──
echo "[CHANGE 3] Setting dangerously low client timeout..."
sed -i 's/timeout client 30s/timeout client 2s/' "$HAPROXY_CFG"
grep 'timeout client' "$HAPROXY_CFG" || true
systemctl reload haproxy 2>/dev/null || true
