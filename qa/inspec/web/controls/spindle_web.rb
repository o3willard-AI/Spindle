...
control 'spindle-web-01' do
  ...
  describe http('http://localhost', headers: { 'Host' => "#{host_inventory['hostname']}.clubhouse.local" }) do
...
control 'spindle-web-02' do
  ...
  let(:headers) { http('http://localhost', headers: { 'Host' => "#{host_inventory['hostname']}.clubhouse.local" }).headers }
