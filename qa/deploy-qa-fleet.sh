...
#
# Node assignments:
#   fleet-01 (192.168.101.211) — spindle-web      (Apache + enterprise portal)
#   fleet-02 (192.168.101.212) — spindle-database  (PostgreSQL + tuning)
#   fleet-03 (192.168.101.213) — spindle-loadbalancer (HAProxy + SSL)
...
DATA_COLLECTOR_URL="${DATA_COLLECTOR_URL:-http://192.168.101.101:8081/ingest/events/data-collector}"
...
# ── Node 1: Web Server ─────────────────────────────────────────────────────
echo "--- fleet-01 (192.168.101.211): spindle-web ---"

ssh_cmd $2 "mkdir -p $TMP_DIR" && scp_cmd "$COOKBOOK_DIR" 192.168.101.211 "$TMP_DIR/"
scp_cmd "$ROLE_DIR/web.json" 192.168.101.211 "$TMP_DIR/role.json"
scp_cmd "$INSPEC_DIR/web" 192.168.101.211 "$TMP_DIR/inspec"

ssh_cmd 192.168.101.211 "
...
echo "--- fleet-02 (192.168.101.212): spindle-database ---"

ssh_cmd $2 "mkdir -p $TMP_DIR" && scp_cmd "$COOKBOOK_DIR" 192.168.101.212 "$TMP_DIR/"
scp_cmd "$ROLE_DIR/database.json" 192.168.101.212 "$TMP_DIR/role.json"
scp_cmd "$INSPEC_DIR/database" 192.168.101.212 "$TMP_DIR/inspec"

ssh_cmd 192.168.101.212 "
    if ! command -v cinc-client &>/dev/null; then
        curl -L https://omnitruck.cinc.sh/install.sh | sudo bash -s -- -v 18
    fi
...
echo "--- fleet-03 (192.168.101.213): spindle-loadbalancer ---"

ssh_cmd $2 "mkdir -p $TMP_DIR" && scp_cmd "$COOKBOOK_DIR" 192.168.101.213 "$TMP_DIR/"
scp_cmd "$ROLE_DIR/loadbalancer.json" 192.168.101.213 "$TMP_DIR/role.json"
scp_cmd "$INSPEC_DIR/loadbalancer" 192.168.101.213 "$TMP_DIR/inspec"

ssh_cmd 192.168.101.213 "
    if ! command -v cinc-client &>/dev/null; then
        curl -L https://omnitruck.cinc.sh/install.sh | sudo bash -s -- -v 18
    fi
...
echo "Monitor: curl -s http://192.168.101.101:8081/health"
echo ""
echo "Next steps:"
