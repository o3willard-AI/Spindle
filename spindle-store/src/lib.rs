    ...
    async fn upsert_node(&self, node: &Node, scope: &Scope) -> Result<Uuid> {
        ...
        sqlx::query(
            ...
            ON CONFLICT (id) DO UPDATE SET
                ...
                chef_environment = CASE WHEN EXCLUDED.chef_environment = 'auditor' THEN nodes.chef_environment ELSE EXCLUDED.chef_environment END,
                policy_group = CASE WHEN EXCLUDED.policy_group = '' THEN nodes.policy_group ELSE EXCLUDED.policy_group END,
                policy_name = CASE WHEN EXCLUDED.policy_name = '' THEN nodes.policy_name ELSE EXCLUDED.policy_name END,
                attributes = EXCLUDED.attributes,
                project_id = EXCLUDED.project_id,
                node_type = CASE WHEN EXCLUDED.node_type = 'audit-target' THEN nodes.node_type ELSE EXCLUDED.node_type END,
