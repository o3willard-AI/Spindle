...
|---------|----------|----------|
| **GitHub PAT embedded in git remote URL** | **P0** | `git remote -v` → `CHANGE_ME@github.com/...` |
| **Live DB credentials hardcoded in 5 source files** | **P0** | `jit_auth.rs:573`, `nodes.rs:1190`, `runs.rs:1339`, `tests/e2e.rs:27`, `main.rs:329` — `postgres://spindle:spindle-dev-password@198.51.100.101:5432/spindle` |
...
- Live DB credentials hardcoded in test files — tests connect to `198.51.100.101:5432` with real password
...
| DB connection string | `spindle-dev-password@198.51.100.101:5432/spindle` in 5 files |
...
|---|---------|----------|--------|--------|----------|
| P0-1 | **GitHub PAT embedded in git remote URL** | Critical | Full repo write access | 30min | `git remote -v` shows `CHANGE_ME...` |
| P0-2 | **Live DB credentials + internal IP in 5 source files** | Critical | DB access to `198.51.100.101:5432` with password | 2h | `jit_auth.rs:573`, `nodes.rs:1190`, `runs.rs:1339`, `tests/e2e.rs:27`, `main.rs:329` |
